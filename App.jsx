...
// --- CONFIGURACIÓN DE FIREBASE (Asegúrate de que estas sean tus llaves) ---
    ...
    <div className="min-h-screen bg-[#f1f5f9] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-white rounded-[2.5rem] shadow-2xl p-8 md:p-12 border border-zinc-200/50 animate-in zoom-in-95 duration-300">
        <div className="text-center mb-10">
          ...
          <h1 className="text-3xl font-black tracking-tighter uppercase italic text-zinc-900">Winner OS</h1>
          <p className="text-zinc-400 text-[10px] font-bold mt-2 tracking-widest uppercase">Cloud Management System</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          ...
          {error && <div className="bg-rose-50 border border-rose-100 p-4 rounded-2xl text-rose-600 text-[11px] font-bold uppercase text-center">⚠️ {error}</div>}
...
export default function App() {
  ...
  if (loading && !user) return <div className="min-h-screen bg-[#f1f5f9] flex items-center justify-center font-bold text-zinc-400 animate-pulse uppercase tracking-[0.3em]">Cargando Sistema...</div>;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 text-zinc-900">
          <div className="space-y-4">
            <div className="aspect-square bg-zinc-50 rounded-xl md:rounded-2xl border-2 border-dashed border-zinc-200 relative flex items-center justify-center overflow-hidden shadow-inner group max-w-sm mx-auto w-full">
              {newProduct.image ? <img src={newProduct.image} className="w-full h-full object-cover"/> : <span className="text-zinc-300 font-bold text-[10px] uppercase">Foto Winner</span>}
            ...
            <textarea value={newProduct.description || ''} onChange={(e)=>setNewProduct({...newProduct, description: e.target.value})} rows={3} className="w-full bg-zinc-50 border border-zinc-100 rounded-xl p-4 text-xs resize-none outline-none" placeholder="Estrategia estratégica..."/>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] p-3 md:p-8 font-sans text-zinc-900 overflow-x-hidden">
      ...
      {notification && (
        <div className="fixed bottom-6 md:bottom-10 left-1/2 transform -translate-x-1/2 bg-zinc-900 text-white px-6 py-3 rounded-xl shadow-2xl z-[200] font-bold text-[10px] md:text-xs uppercase tracking-widest animate-in slide-in-from-bottom-10 w-[90%] md:w-auto text-center">
          ✨ {notification}
        </div>
      )}
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4 bg-white p-4 md:p-6 rounded-2xl md:rounded-[2rem] shadow-sm border border-zinc-200/50 relative">
          <div className="flex items-center gap-3 md:gap-5 w-full md:w-auto">
            ...
            <div>
              ...
              <p className="text-[8px] md:text-[10px] text-zinc-400 mt-1 uppercase font-black tracking-widest md:tracking-[0.3em]">{loading ? 'Cargando...' : 'Cloud Real-Time'}</p>
            </div>
          </div>
              ...
              <div key={p.id} className="bg-white rounded-[1.5rem] md:rounded-[3rem] shadow-sm border border-zinc-200/50 overflow-hidden transition-all hover:shadow-xl animate-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="flex flex-col xl:flex-row">
                   <div className="w-full xl:w-[25%] p-5 md:p-10 border-r border-zinc-100 bg-zinc-50/10">
                        ...
                        {isWinner ? (
                          <>
                            ...
                            <div className="bg-white border border-zinc-100 p-2 md:p-3 rounded-xl md:rounded-2xl shadow-sm">
                                <label className="text-[7px] md:text-[8px] font-black text-zinc-400 uppercase tracking-widest block mb-1 leading-none">PROVEEDOR</label>
                          ...
                          <>
                            <div className="bg-white border border-zinc-100 p-2 md:p-3 rounded-xl md:rounded-2xl shadow-sm">
                                <label className="text-[7px] md:text-[8px] font-black text-zinc-400 uppercase tracking-widest block mb-1 leading-none text-left">CHINESE PROV</label>
                     ...
                     {isWinner && (
                       <textarea value={p.description || ''} onChange={(e)=>updateDocField(p.id, 'description', e.target.value)} rows={3} className="w-full text-[10px] md:text-xs bg-white p-4 md:p-6 rounded-xl md:rounded-[1.5rem] border border-zinc-100 shadow-inner text-zinc-500 leading-relaxed" placeholder="Estrategia..."/>
                     )}
                   </div>
                      ...
                      {isWinner ? (
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 md:gap-4">
                            ...
                            <div className="col-span-2 bg-zinc-50 p-3 md:p-5 rounded-xl md:rounded-2xl border border-zinc-100 text-left">
                                <label className="text-[8px] md:text-[10px] font-black text-zinc-400 uppercase block mb-2 px-1">Dimensiones (cm)</label>
                            ...
                            <div className="relative z-10 space-y-6 md:space-y-8">
                                ...
                                <div className="flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0">
                                    ...
                                    <div className="text-center md:text-right md:ml-10">
                                        <p className="text-[8px] md:text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-1 md:mb-2 italic leading-none">Margen ROI</p>
                            ...
                            <div className="relative z-10 space-y-6 md:space-y-8">
                                ...
                                <div className="bg-emerald-500/10 p-5 md:p-10 rounded-xl md:rounded-[3rem] border border-emerald-500/20 flex flex-col md:flex-row justify-between items-center gap-6 md:gap-10 transition-all hover:bg-emerald-500/20">
                                    <div className="flex items-center gap-4 md:gap-8 flex-col md:flex-row text-center md:text-left w-full md:w-auto">
                                        ...
                                        <div className="text-left">
                                            <p className="text-[10px] md:text-[12px] font-black text-emerald-400 uppercase tracking-widest mb-1 md:mb-2 leading-none">Costo Producto Colombia</p>
                                            <p className="text-4xl md:text-7xl font-black text-white leading-none">{formatCurrency(mImport.unitCostColombia)}</p>
                                        </div>
                                    </div>
                      ...
                      {!isWinner && p.status === 'approved' && (
                        <div className="pt-6 md:pt-8 border-t border-zinc-100">
                           <button onClick={()=>setExpandedItems({...expandedItems, [p.id]: !expandedItems[p.id]})} className={`w-full p-4 md:p-8 rounded-xl md:rounded-[2.5rem] border-2 transition-all flex justify-between items-center ${expandedItems[p.id] ? 'bg-zinc-900 border-zinc-900 text-white shadow-xl' : 'bg-zinc-50 border-zinc-200 text-zinc-900'}`}>
                                <div className="flex items-center gap-3 md:gap-5"><span className="text-xl md:text-3xl">📋</span><div className="text-left"><p className="text-[10px] md:text-sm font-black uppercase tracking-widest leading-none">Concepto Compra / Logística</p></div></div>
                                    ...
                                    <div className="space-y-4">
                                        <div className="grid grid-cols-2 gap-4 text-left">
                                            <div className="flex flex-col gap-1">
                                                ...
                                                <input type="date" value={p.purchaseDate || ''} onChange={(e)=>updateDocField(p.id, 'purchaseDate', e.target.value)} className="w-full p-3 rounded-xl border text-[10px] md:text-sm font-mono focus:border-zinc-900 outline-none"/>
                                            </div>
                                            <div className="flex flex-col gap-1">
                                                <label className="text-[9px] font-bold text-zinc-400 uppercase px-1">Est. Llegada</label>
                                                <input type="date" value={p.estimatedArrival || ''} onChange={(e)=>updateDocField(p.id, 'estimatedArrival', e.target.value)} className="w-full p-3 rounded-xl border text-[10px] md:text-sm font-mono focus:border-zinc-900 outline-none"/>
                                            </div>
                                        </div>
                                        <div className="flex flex-col gap-1 text-left">
                                            <label className="text-[9px] font-bold text-zinc-400 uppercase px-1">Valor Anticipo</label>
                                            <input type="number" value={p.advancePayment || 0} onChange={(e)=>updateDocField(p.id, 'advancePayment', parseFloat(e.target.value)||0)} className="w-full p-3 rounded-xl border text-[10px] md:text-sm font-mono font-bold outline-none"/>
                                        </div>
                                        <div className="flex flex-col gap-1 text-left">
                                            <label className="text-[9px] font-bold text-zinc-400 uppercase px-1">Comprador</label>
                                            <input value={p.buyer || ''} onChange={(e)=>updateDocField(p.id, 'buyer', e.target.value)} className="w-full p-3 rounded-xl border text-[10px] md:text-sm font-bold outline-none" placeholder="Nombre..."/>
                                        </div>
                                    </div>
                                    <div className="bg-white p-4 md:p-8 rounded-2xl border shadow-sm text-left">
                                        <h4 className="text-[10px] font-black text-zinc-400 uppercase mb-4 border-b pb-2 px-1">Distribución por Color</h4>
                                            ...
                                            {(p.colors || []).map(c => (
                                                <div key={c.id} className="flex gap-2 md:gap-4 items-center bg-zinc-50/50 p-2 rounded-xl border">
                                                    ...
                                                    <input type="number" value={c.qty || 0} onChange={(e)=>{
                                                        ...
                                                        updateDocField(p.id, 'colors', n);
                                                    }} className="w-16 md:w-24 bg-zinc-900 text-white p-1.5 rounded-lg text-center text-[10px] md:text-xs font-mono shadow-sm"/>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                   ...
                                   <div key={u.id} className="bg-white p-4 rounded-2xl border border-zinc-200 flex gap-4 relative group border-l-8 border-l-indigo-600 transition-all">
                                       ...
                                       <div className="flex-1 min-w-0 pr-6">
                                           ...
                                           <div className="flex gap-2">
                                               ...
                                               <input type="number" value={u.price || ''} onChange={(e)=>updateUpsell(p, u.id, 'price', parseFloat(e.target.value)||0)} className="w-1/2 bg-indigo-50/50 text-[9px] p-1.5 rounded-lg font-black text-indigo-700 outline-none border border-transparent shadow-inner" placeholder="Venta"/>
                                           </div>
                                       </div>
      ...
      {/* MODAL CREACIÓN */}
      {isCreating && (
        <div className="fixed inset-0 bg-zinc-950/90 backdrop-blur-xl flex items-center justify-center z-[300] p-3">
